...
const filter = () => {
    ...
    const [selectd, setSelected] = useState<Category[]>([])
    const flexWidth = useSharedValue(0)
    const scale = useSharedValue(0)

    useEffect(() => {
        const hasSelected = selectd.length > 0
        const selectedItems = items.filter((item) => item.checked)
        const newSelectd = selectedItems.length > 0

        if(hasSelected !== newSelectd) {
            flexWidth.value = withTiming(newSelectd ? 150 : 0)
            scale.value = withTiming(newSelectd ? 1 : 0)
        }
